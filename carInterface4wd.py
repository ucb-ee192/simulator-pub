...
class Car(object):
  ...
  def __init__(self, vrep_interface: Any, steering_limit: float = 30, steering_slew_rate: float = (60/0.016)) -> None:
    ...
    self.motor_rl_handle = self.vr.simxGetObjectHandle('RearLeftMotor', vrep.simx_opmode_oneshot_wait)
    self.motor_rr_handle = self.vr.simxGetObjectHandle('RearRightMotor', vrep.simx_opmode_oneshot_wait)
    self.vr.simxSetJointTargetVelocity(self.motor_rl_handle, 0, vrep.simx_opmode_oneshot_wait)
    self.vr.simxSetJointTargetVelocity(self.motor_rr_handle, 0, vrep.simx_opmode_oneshot_wait)
  ...
  def set_speed(self, speed: float, blocking: bool = False) -> None:
    ...
    self.vr.simxSetJointTargetVelocity(self.motor_rl_handle, desired_speed, op_mode)
    self.vr.simxSetJointTargetVelocity(self.motor_rr_handle, desired_speed, op_mode)
